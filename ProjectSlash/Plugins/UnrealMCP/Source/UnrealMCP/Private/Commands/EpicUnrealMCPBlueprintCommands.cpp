...
#include "UObject/UObjectGlobals.h" // FindFirstObject, StaticLoadClass

namespace
{
    // Resolve a component class name to UClass* without using deprecated ANY_PACKAGE
    static UClass* ResolveComponentClassByName(const FString& InComponentType)
    {
        auto TryFindBySimpleName = [](const FString& Name) -> UClass*
        {
            if (Name.IsEmpty()) return nullptr;
            return FindFirstObject<UClass>(*Name);
        };

        // 1) Exact name as given
        if (UClass* Cls = TryFindBySimpleName(InComponentType))
        {
            return Cls;
        }

        // 2) With "Component" suffix
        if (!InComponentType.EndsWith(TEXT("Component")))
        {
            const FString WithSuffix = InComponentType + TEXT("Component");
            if (UClass* Cls = TryFindBySimpleName(WithSuffix))
            {
                return Cls;
            }
        }
        ...
        // 3) With leading 'U'
        if (!InComponentType.StartsWith(TEXT("U")))
        {
            const FString WithUPrefix = TEXT("U") + InComponentType;
            if (UClass* Cls = TryFindBySimpleName(WithUPrefix))
            {
                return Cls;
            }

            // 4) With both 'U' prefix and 'Component' suffix
            if (!InComponentType.EndsWith(TEXT("Component")))
            {
                const FString WithBoth = TEXT("U") + InComponentType + TEXT("Component");
                if (UClass* Cls = TryFindBySimpleName(WithBoth))
                {
                    return Cls;
                }
            }
        ...
        // 5) Try StaticLoadClass from common script modules using only the short name
        //    This can succeed for engine components if module and class are exported.
        const TCHAR* CommonModules[] = {
            TEXT("Engine"), TEXT("UMG"), TEXT("Niagara"), TEXT("NavigationSystem"), TEXT("AIModule"), TEXT("GameplayTasks"), TEXT("EnhancedInput")
        };

        auto TryLoadFromModules = [](const FString& ShortName, const TArray<FString>& Candidates) -> UClass*
        {
            for (const FString& Module : Candidates)
            {
                const FString Path = FString::Printf(TEXT("/Script/%s.%s"), *Module, *ShortName);
        ...
        // Build candidate class names to try loading (as given, +Component, U+Name, U+Name+Component)
        TArray<FString> CandidateNames;
        CandidateNames.Add(InComponentType);
        if (!InComponentType.EndsWith(TEXT("Component")))
        {
            CandidateNames.Add(InComponentType + TEXT("Component"));
        }
        if (!InComponentType.StartsWith(TEXT("U")))
        {
            CandidateNames.Add(TEXT("U") + InComponentType);
            if (!InComponentType.EndsWith(TEXT("Component")))
            {
                CandidateNames.Add(TEXT("U") + InComponentType + TEXT("Component"));
            }
        }

        TArray<FString> Modules;
        for (const TCHAR* M : CommonModules)
        {
            Modules.Add(M);
        }

        for (const FString& ShortName : CandidateNames)
        {
            if (UClass* Loaded = TryLoadFromModules(ShortName, Modules))
            {
                return Loaded;
            }
        }
...
TSharedPtr<FJsonObject> FEpicUnrealMCPBlueprintCommands::HandleAddComponentToBlueprint(const TSharedPtr<FJsonObject>& Params)
{
    ...
    // Create the component - dynamically find the component class by name (UE5.7-safe)
    UClass* ComponentClass = ResolveComponentClassByName(ComponentType);
